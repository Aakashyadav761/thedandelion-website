...
export default function UnitSection({ unit }: { unit: UnitData }) {
  ...
  return (
    <section className="bg-cream border-b border-earthen/15 overflow-hidden">
      {/*
        Full-bleed flex layout:
        - Image column: 52% of section width, no outer horizontal padding
        - Content column: fills the rest, with its own internal padding
        - items-stretch makes both columns the same height so bottoms align
      */}
      <div
        className={`flex flex-col items-stretch min-h-[520px] lg:min-h-[580px] ${
          flipped ? "lg:flex-row-reverse" : "lg:flex-row"
        }`}
      >
        {/* Image column — full bleed, no horizontal margin */}
        <div className="w-full lg:w-[52%] flex-shrink-0 flex flex-col min-h-[260px]">
          <UnitGallery images={unit.images} />
        </div>

        {/* Content column — internal padding, CTA bottom-aligned */}
        <div className="flex-1 flex flex-col px-5 sm:px-8 lg:px-10 xl:px-14 py-10 md:py-12">
          ...
          {/* Spacer — pushes CTA to the bottom of the column */}
          <div className="flex-1 min-h-[16px]" />

          {/* CTA — bottom-aligned with the thumbnail strip */}
